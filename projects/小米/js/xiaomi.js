...
// 家电
let ccc=document.querySelectorAll(".jiadian")
for(let i=0;i<ccc.length;i++){
    bbb(ccc[i]);
}
function bbb(lll) {
    let obox=lll.querySelectorAll(".jiadian-daohang");
    let obx=lll.querySelectorAll(".jiadian-dadahz");
    obox.forEach(function (val,index){
        val.onmouseover=function () {
            for(let i=0;i < obox.length;i++){
                obox[i].classList.remove("jiadian-daohang1");
                obx[i].style.display="none";
            }
            val.classList.add("jiadian-daohang1");
            obx[index].style.display="block";
        }
    })
}
...
let popo=document.querySelector(".daohang-sousuo");
let pop=document.querySelector(".piaofu");
popo.onfocus=function () {
    pop.style.display="none";
}
popo.onblur=function () {
